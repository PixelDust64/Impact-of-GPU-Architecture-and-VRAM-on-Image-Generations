...
# ====================================================================================
# SUB-QUADRATIC ATTENTION IMPLEMENTATION (Otimizado para alto Throughput/Largura de Banda)
# ====================================================================================
def attention_sub_quad(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    attn_precision = get_attn_precision(attn_precision, q.dtype)
    if skip_reshape:
        b, _, _, dim_head = q.shape
    else:
    ...
    if skip_reshape:
        query = q.reshape(b * heads, -1, dim_head)
        value = v.reshape(b * heads, -1, dim_head)
        key = k.reshape(b * heads, -1, dim_head).movedim(1, 2)
    else:
        query = q.unsqueeze(3).reshape(b, -1, heads, dim_head).permute(0, 2, 1, 3).reshape(b * heads, -1, dim_head)
        value = v.unsqueeze(3).reshape(b, -1, heads, dim_head).permute(0, 2, 1, 3).reshape(b * heads, -1, dim_head)
        key = k.unsqueeze(3).reshape(b, -1, heads, dim_head).permute(0, 2, 3, 1).reshape(b * heads, dim_head, -1)
    
    dtype = query.dtype
    upcast_attention = attn_precision == torch.float32 and query.dtype != torch.float32
    bytes_per_token = 4 if upcast_attention else query.element_size()
    batch_x_heads, q_tokens, _ = query.shape
    _, _, k_tokens = key.shape
    
    # Cálculo agressivo de memória livre para Radeon Pro VII
    try:
        mem_free_total, _ = memory_management.get_free_memory(query.device, True)
    except:
        # Fallback se get_free_memory falhar
        mem_free_total = torch.cuda.mem_get_info()[0]

    kv_chunk_size, query_chunk_size = None, None
    
    # Tentativa de alocar chunks maiores primeiro, aproveitando a largura de banda da Radeon Pro VII
    for x in [8192, 4096, 2048, 1024, 512, 256]:
        if (mem_free_total / (batch_x_heads * bytes_per_token * x * 4.0)) >= k_tokens:
            kv_chunk_size, query_chunk_size = k_tokens, x
            break
            
    query_chunk_size = query_chunk_size or 512
    ...
    if skip_output_reshape:
        return hidden_states.to(dtype).unflatten(0, (-1, heads))
        
    return hidden_states.to(dtype).unflatten(0, (-1, heads)).transpose(1, 2).flatten(start_dim=2)


def attention_split(q, k, v, heads, mask=None, attn_precision=None, skip_reshape=False, skip_output_reshape=False, **kwargs):
    attn_precision = get_attn_precision(attn_precision, q.dtype)
    if skip_reshape: b, _, _, dim_head = q.shape
    else:
        b, _, dim_head = q.shape
        dim_head //= heads
    ...
    if skip_reshape: q, k, v = map(lambda t: t.reshape(b * heads, -1, dim_head), (q, k, v))
    else: q, k, v = map(lambda t: t.unsqueeze(3).reshape(b, -1, heads, dim_head).permute(0, 2, 1, 3).reshape(b * heads, -1, dim_head).contiguous(), (q, k, v))
    
    r1 = torch.zeros(q.shape[0], q.shape[1], v.shape[2], device=q.device, dtype=q.dtype)
    mem_free_total = memory_management.get_free_memory(q.device)
    element_size = 4 if attn_precision == torch.float32 else q.element_size()
    
    steps = 1
    mem_required = (q.shape[0] * q.shape[1] * k.shape[1] * element_size) * 3
    if mem_required > mem_free_total: steps = 2 ** (math.ceil(math.log(mem_required / mem_free_total, 2)))
    ...
    while True:
        try:
            ...
            for i in range(0, q.shape[1], slice_size):
                ...
                s1 = torch.einsum("b i d, b j d -> b i j", q[:, i:end].float() if element_size==4 else q[:, i:end], k.float() if element_size==4 else k) * scale
                if mask is not None: s1 += mask[i:end] if mask.ndim==2 else mask[:, i:end]
    ...
    if skip_output_reshape: return r1.unsqueeze(0).reshape(b, heads, -1, dim_head)
    return r1.unsqueeze(0).reshape(b, heads, -1, dim_head).permute(0, 2, 1, 3).reshape(b, -1, heads * dim_head)


# =========================================================================
# LÓGICA DE SELEÇÃO DE ATENÇÃO (AGORA CORRIGIDA)
# =========================================================================
...
# REMOVA OU COMENTE A LINHA ABAIXO QUE CAUSA O ERRO
# elif args.attention_split: 
#     logger.info("Using Attention Split")
#     attention_function = attention_split
    
else:
    logger.info("Using Sub Quadratic Optimization for Cross Attention (Optimized for Radeon Pro VII)")
